...
``[options.entry_points]`` section in ``setup.cfg``::
    ...
    console_scripts =
         fibonacci = solawi_teilnehmer_auswertung.skeleton:run
...
def parse_args(args):
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="solawi-teilnehmer-auswertung {ver}".format(ver=__version__),
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
if __name__ == "__main__":
    ...
    #
    #     python -m solawi_teilnehmer_auswertung.skeleton 42
    #
    run()
